...
export default function Location() {
  ...
  return (
    <section id="location" ref={sectionRef} className="py-24 bg-cream relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-10 items-center">
          ...
          <div className="reveal-left">
                ...
                <div className="space-y-3">
                  {t.address.map((line, i) => (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <span className="font-poppins text-gray-700 text-base">{line}</span>
                    </div>
                  ))}
                </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-6">
              {[t.hours, t.days].map(({ emoji, title, detail }) => (
                <div key={title} className="bg-white rounded-2xl p-4 shadow-md border border-orange-100 text-center">
                  <div className="text-2xl mb-2">{emoji}</div>
                  <p className="font-playfair text-maroon font-bold text-sm">{title}</p>
                  <p className="font-poppins text-gray-500 text-xs mt-1">{detail}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="reveal-right">
            <div className="rounded-3xl overflow-hidden shadow-2xl border-4 border-white" style={{ height: '420px' }}>
              <iframe
